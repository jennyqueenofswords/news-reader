...
- **2026-07-09** — First real miss the (working) alarm caught. Cause was neither TCC nor publish — the cardpuller hit its **$2 `--max-budget-usd` ceiling and was killed before drawing** (folding publish into the run made $2 too tight). Note: the studio runs on **Jenny's Claude Code subscription, not a metered API key** — so that ceiling is a self-imposed *estimate* guardrail against eating plan quota, NOT a dollar bill. Raised it to **$4** (a ceiling, not a target — free headroom). Decisions (Jenny): **7/8 left as an honest gap** (no backfill — matches the no-fabrication rule), **7/9 drawn by tonight's scheduled run** (not forced early). Added a **snooze** to `check-news-reader.sh` (`reports/SNOOZE-news-reader` = a date; stay quiet through it, auto-re-arm after) and snoozed through 7/9 so the accepted one-day gap doesn't ping hourly. Real test: tonight's run completing under $4.
